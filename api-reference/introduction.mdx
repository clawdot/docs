...
<Note>
  API Key 与 Agent 在 [Portal](http://console.hicaspian.com/agents) 创建。`consent_grant_id`（`cg_` 前缀）由用户绑定流程返回，详见 [认证机制](/gateway/authentication)。
</Note>
...
网关路径（REST `/api/v1` 与 MCP）的错误响应会带上 `request_id`——本次请求的追踪 UUID（可选字段，客户端应容忍其缺省）；**报障时请一并提供**，运维可凭它直达日志。
