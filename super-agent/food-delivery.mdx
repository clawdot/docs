...
Super Agent 在外卖场景下的核心能力（每个能力对应一个 MCP 工具，共 18 个工具）：
...
<AccordionGroup>
  <Accordion title="使用默认规格" icon="zap">
  ...
  <Accordion title="地址管理策略" icon="map-pin">
    地址管理是提升用户体验的关键：
  ...
  <Accordion title="错误处理" icon="triangle-alert">
    外卖场景中常见的错误和处理方式：
...
<CardGroup cols={2}>
  ...
  <Card title="❌ 避免做法" icon="circle-x" color="red">
    - 直接创建而不预览
    - 让用户手动处理规格互斥
    - 每次都重新搜索地址
    - 返回技术错误码给用户
...
<Note>
  认证为双层：`Authorization: Bearer clw_...` 标识 Agent；用户授权（`cg_` 前缀的 consent grant）在 MCP 工具里作为 `consent_grant_id` 参数传入。MCP 公开端点为 `/mcp/v1`。创建 Agent、签发 API Key 走 [Portal](https://console.hicaspian.com/agents)。
</Note>
