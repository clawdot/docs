...
graph LR
  REST["REST API\n/api/v1/*"] --> Services
  MCP["MCP Server\n/mcp"] --> Services
  Services --> Client["外卖 Client"] --> API["外卖平台 API"]
...
<AccordionGroup>
  ...
  <Accordion title="Service 层 — 业务逻辑" icon="gears">
    **位置：** `src/services/`
    ...
    核心业务逻辑所在，通过构造函数接收依赖（DB session, 外卖 client）：
  ...
  <Accordion title="外卖 Client 层 — API 封装" icon="cloud">
    **位置：** `src/client/`

    外卖平台 API 的 HTTP 客户端封装：
...
sequenceDiagram
  ...
  par 10 个并行搜索
    Shop->>Platform: searchByKeyWords("推荐")
    Shop->>Platform: searchByKeyWords("快餐")
    Shop->>Platform: searchByKeyWords("咖啡")
    Shop->>Platform: ...（共 10 个品类）
  end
  Platform-->>Shop: 各品类结果
...
sequenceDiagram
  ...
  Order->>Platform: 第 1 次渲染（无优惠券）
  Platform-->>Order: 可用优惠券列表
  Order->>Order: 选择最优优惠券
  Order->>Platform: 第 2 次渲染（带优惠券）
  Platform-->>Order: 最终价格
...
<Tabs>
  ...
  <Tab title="orders">
    ...
    | platform_order_id | varchar(64) | 外卖订单 ID |
