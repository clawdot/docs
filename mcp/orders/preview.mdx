...
| `items[].selected_ingredients` | array | 该项已选配料/做法，`[{group_name, name, price_delta, quantity}]`；`group_name` 可为 `null`，`price_delta` 为**单份**加价（分），`quantity` 为生效份数（默认 `1`，该项实际加价 = `price_delta × quantity`）；无则为 `[]` |
