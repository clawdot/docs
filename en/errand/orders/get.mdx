...
| `payment_expire_at` | string \| null | Payment deadline (Beijing time), 15 minutes after the order is placed; past that point the order may be closed automatically at any moment. **Only unpaid orders have a value** — `null` for every other status |
