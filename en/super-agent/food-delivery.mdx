...
Super Agent's core capabilities in food delivery scenarios (each capability corresponds to one MCP tool — 18 tools in total):
...
<AccordionGroup>
  <Accordion title="Use Default Specs" icon="zap">
  ...
  <Accordion title="Address Management Strategy" icon="map-pin">
...
<CardGroup cols={2}>
  ...
  <Card title="❌ Avoid" icon="circle-x" color="red">
...
<Note>
  Authentication is two-layered: `Authorization: Bearer clw_...` identifies the Agent; the user authorization (a `cg_`-prefixed consent grant) is passed as the `consent_grant_id` argument in MCP tools. The public MCP endpoint is `/mcp/v1`. Creating an Agent and issuing API Keys is done in the [Portal](https://console.hicaspian.com/agents).
</Note>
