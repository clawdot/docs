...
| `items[].selected_ingredients` | Selected ingredients / options, `[{group_name, name, price_delta}]`, e.g. `[{"group_name":"Add-ons","name":"Boba","price_delta":200}]`; `group_name` may be `null`, `price_delta` is the surcharge for one portion (cents); **never multiply it by the portion count** — see `price_steps` on the option candidates for tiered pricing |
