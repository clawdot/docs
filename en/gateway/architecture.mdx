...
<Note>
  This page only covers the **public** contract: the `/api/v1/*` REST endpoints and the `/mcp/v1` MCP tools. Operational actions such as creating an Agent, issuing an API Key, and enabling a capability go through the Portal ([Clawdot Portal](http://console.hicaspian.com/agents)) and are not part of the public API.
</Note>
...
<Tabs>
  <Tab title="REST API">
    ...
    See the [API Reference](/en/api-reference/user/bind-request).
  </Tab>
