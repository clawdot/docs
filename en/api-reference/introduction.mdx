...
<Note>
  API Keys and Agents are created in the [Portal](http://console.hicaspian.com/agents). The `consent_grant_id` (`cg_` prefix) is returned by the user binding flow — see [Authentication](/en/gateway/authentication).
</Note>
...
Gateway error responses (REST `/api/v1` and MCP) include a `request_id` — a per-request trace UUID (optional; clients should tolerate its absence). **Quote it when reporting an issue** so the team can jump straight to the logs.
