...
| `can_checkout` | boolean | Whether the cart is checkout-ready (`false` if any item is not checkout-able, a store-level required group is unmet, an item's quantity is below its minimum purchase, or upstream blocks checkout) |
