...
Responses can be returned in the language you choose: shop names, item names, specs and add-ons, labels, order status and error messages are all localized. One of `zh` (Chinese, default) / `en` / `ja` / `ko` / `ru` / `ms`; any other value returns 400.
