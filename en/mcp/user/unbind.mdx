...
description: "MCP tool: revoke the current user's authorization; clearing the credential invalidates the old consent_grant immediately"
---
...
| `reset_history` | boolean | No | Default `false`. When `true`, revoke and retire the binding: after the same phone number re-binds, previously saved addresses and order history are no longer visible. Irreversible |
...
| `revoked_scopes` | string[] | Capabilities revoked by this call. Customers with cross-capability sharing enabled share one grant between delivery and errand, so both are revoked; otherwise only delivery |
