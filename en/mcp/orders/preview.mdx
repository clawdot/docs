...
| `items[].selected_ingredients` | array | Selected ingredients/options, `[{group_name, name, price_delta, quantity}]`; `group_name` may be `null`, `price_delta` is the **per-portion** surcharge (cents), `quantity` is the effective portion count (default `1`; the line's surcharge is `price_delta × quantity`); `[]` if none |
